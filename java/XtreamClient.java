...
public class XtreamClient {
    ...
    public void fetchVodStreamsByCategory(String categoryId, MoviesCallback callback) {
        if (currentCredential == null) {
            callback.onError("Credenciais não carregadas");
            return;
        }
        ...
        if (categoryId != null && !categoryId.isEmpty() && !categoryId.equals("TODOS")) {
        ...
        httpClient.newCall(request).enqueue(new Callback() {
            @Override
            public void onFailure(Call call, IOException e) {
                Log.e(TAG, "Error fetching movies by category", e);
                callback.onError("Erro ao buscar filmes: " + e.getMessage());
            }
            ...
            public void onResponse(Call call, Response response) throws IOException {
                ...
                try {
                    ...
                    Log.d(TAG, "Successfully fetched " + movies.size() + " movies for category: " + categoryId);
                    callback.onSuccess(movies);
                } catch (Exception e) {
                    Log.e(TAG, "Error parsing movies by category", e);
                    callback.onError("Erro ao processar filmes: " + e.getMessage());
                }
            }
        });
    ...
    public void fetchSeriesByCategory(String categoryId, SeriesCallback callback) {
        if (currentCredential == null) {
            callback.onError("Credenciais não carregadas");
            return;
        }
        ...
        httpClient.newCall(request).enqueue(new Callback() {
            ...
            public void onResponse(Call call, Response response) throws IOException {
                ...
                try {
                    ...
                    Log.d(TAG, "Successfully fetched " + series.size() + " series for category: " + categoryId);
                    callback.onSuccess(series);
                } catch (Exception e) {
                    Log.e(TAG, "Error parsing series by category", e);
                    callback.onError("Erro ao processar séries: " + e.getMessage());
                }
            }
        });
    ...
    public void fetchSeriesInfo(String seriesId, EpisodesCallback callback) {
        Log.d(TAG, "Attempting to fetch series info for ID: " + seriesId);
        
        if (currentCredential == null) {
            Log.e(TAG, "No credentials available for fetching series info");
            callback.onError("Credenciais não carregadas");
            return;
        }
        ...
        Log.d(TAG, "Series info URL: " + url.replaceAll("password=[^&]*", "password=***"));
        ...
        httpClient.newCall(request).enqueue(new Callback() {
            ...
            public void onResponse(Call call, Response response) throws IOException {
                ...
                try {
                    String jsonData = response.body().string();
                    Log.d(TAG, "Series info JSON response (length: " + jsonData.length() + "): " + 
                    ...
                    if (jsonObject.has("episodes")) {
                        ...
                        Log.i(TAG, "Successfully parsed " + totalEpisodes + " episodes for series " + seriesId);
                    } else {
