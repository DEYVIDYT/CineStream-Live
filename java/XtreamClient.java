        ...
        httpClient.newCall(request).enqueue(new Callback() {
            ...
            public void onResponse(Call call, Response response) throws IOException {
                ...
                try {
                    ...
                    if (jsonData.contains("error") || jsonData.contains("Error") || jsonData.contains("not found")) {
                        Log.e(TAG, "Error response from API: " + jsonData);
                        callback.onError("Erro na API: " + jsonData.substring(0, Math.min(jsonData.length(), 200)));
                        return;
                    }
